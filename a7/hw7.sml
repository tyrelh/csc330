      ...
      and (2) v2. Then use that result to compute what we need. *)
	    (case intersect(two_points_to_line seg, v2) of
	      ...
	      | Point(x0,y0) => (* see if the point is within the segment bounds *)
	        ...
	        let 
		        fun inbetween(v,end1,end2) = (end1 - epsilon <= v andalso v <= end2 + epsilon) orelse (end2 - epsilon <= v andalso v <= end1 + epsilon)
		        val (x1,y1,x2,y2) = seg
	        in
  ...
  case e of
    ...
    | Shift(dX, dY, ex) =>
      case ex of
        ...
        | Var s => (
          case List.find (fn (s2,v) => s=s2) env of (* lookup copied from above *)
            NONE => raise BadProgram("var not found: " ^ s)
	        | SOME (_,v) => eval_prog(Shift(dX, dY, v), env))
        | _ => ex
